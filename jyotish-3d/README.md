...
Jyotish 3D is a guided educational chart reader, not a technical dashboard. The app teaches the six-layer reading sequence as a playable progression:

1. Grahas: who acts
2. Rāśis: how the graha behaves
3. Bhāvas: where the action lands
4. Nakṣatras: why the movement has its subtler motive
5. Daśās: when a planetary storyline is active
6. Gocāra: what the current sky triggers

The current UX uses progressive disclosure so the user learns one layer at a time. Each stage changes both the camera and the scene: grahas appear as active agents with a Navagraha Puruṣa cue, rāśis add an elemental atmosphere ring, bhāvas rise as occupied rooms, nakṣatras expose the 27-part lunar ring, daśās show a time ring, and gocāra overlays live transit pressure. Stage changes clear camera focus so selected planets do not drag the next layer off-frame.
