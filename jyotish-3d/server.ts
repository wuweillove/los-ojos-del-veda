...
const JYOTISH_API = "http://localhost:52795";

async function proxyGet(c: any, path: string) {
  const url = new URL(`${JYOTISH_API}${path}`);
  const params = c.req.queries ? c.req.queries() : {};
  Object.entries(params).forEach(([k, vs]) => {
    (vs as string[]).forEach(v => url.searchParams.append(k, v));
  });
  ...
  try {
    ...
    return c.body(data, res.status, { "Content-Type": "application/json" });
  } catch {
    return c.json({ error: "Backend unavailable" }, 503);
  }
}

app.get("/api/calculate", (c) => proxyGet(c, "/api/calculate"));
app.get("/api/transits", (c) => proxyGet(c, "/api/transits"));
app.get("/api/search-city", (c) => proxyGet(c, "/api/search-city"));
app.get("/api/timezone", (c) => proxyGet(c, "/api/timezone"));
