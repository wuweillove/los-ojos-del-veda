...
// Native binary loader with a prebuilds/linux-x64 fallback. Used by both the
// CommonJS (index.js) and ESM (index.mjs) patches so that on Vercel's Linux
// runtime the prebuilt sweph.node is loaded even when node-gyp-build fails.

const cjsPath = path.join(process.cwd(), 'node_modules/sweph/index.js');
const cjsContent = `/// <reference path="index.d.ts"/>
"use strict";
...
const mjsPath = path.join(process.cwd(), 'node_modules/sweph/index.mjs');
const mjsContent = `import { createRequire } from "module";
...
try {
    ...
    const binaryPath = join(__dirname, "prebuilds", "linux-x64", "sweph.node");
    try {
        s = require(binaryPath);
    } catch (e2) {
        console.error("Failed to load sweph native binary from:", binaryPath);
...
if (fs.existsSync(cjsPath)) {
    ...
    console.error('sweph/index.js not found at:', cjsPath);
    ok = false;
}

if (fs.existsSync(mjsPath)) {
    fs.writeFileSync(mjsPath, mjsContent);
    console.log('Successfully patched sweph/index.mjs');
} else {
    console.error('sweph/index.mjs not found at:', mjsPath);
    ok = false;
}
